...
#define ID_MAX_SIZE (256 + 1)	// Maximum size of identifer
...
typedef struct Declaration{
    DataType type;
//    char name;	// Change has made here !!!
...
typedef struct Value{
    ValueType type;
    union{
//        char id;   // Changes have made here!!!                /* if the node represent the access of the identifier */
		char id[ID_MAX_SIZE];
...
Expression *parseTerm(FILE *, Expression *);

// Statement makeAssignmentNode( char id, Expression *v, Expression *expr_tail );
Statement makeAssignmentNode(char *id, Expression *v, Expression *exp_tail);
...
void constant_folding(Expression *);
void print_expr( Expression *expr );
void test_parser( FILE *source );
void print_code(Program *);
